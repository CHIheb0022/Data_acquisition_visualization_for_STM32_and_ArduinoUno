...
try:
    ...
    # Count existing files for the category label within the output directory (named as the label) 
    existing_files = glob.glob(os.path.join(out_dir, f"{label}.*.csv"))  
    i = len(existing_files)
    pass    
else:
    #Initialize a counter (Used to Collect balanced dataset size for different classes)
    i = 0 
        ...
        if ser.in_waiting > 0:
            while(ser.in_waiting):
                ...
                if rx_buf[-4:] == b'\r\n\r\n':
                    ...
                    i+=1
                    print(f"{i} Examples collected for {label} Category")
